...
public class ValidatorTest {
  ...
  public void validateCloverId5() {
    ...
    x.setItem(new Reference().setId("1")); // reference too short
    x.validate();

    assertTrue(strictValidationFailed.get());
  }

  @Test
  public void validateCloverId6() {
    OrderType x = new OrderType();
    // reference in list too short
    x.setCategories(Arrays.asList(new Reference().setId("1234567890123"), new Reference().setId("1")));
    x.validate();

    assertTrue(strictValidationFailed.get());
  }

  @Ignore("The server and client sometimes use bad Clover IDs")
  @Test(expected = IllegalArgumentException.class)
